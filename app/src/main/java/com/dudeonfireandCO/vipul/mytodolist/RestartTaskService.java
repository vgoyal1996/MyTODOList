...
public class RestartTaskService extends IntentService {
    public static final String RESTART_SERVICE_EXTRA_ROW = "row";

    public RestartTaskService() {
        super("RestartTaskService");
    }

    @Override
    protected void onHandleIntent(Intent intent) {
        int row = (int)intent.getExtras().get(RESTART_SERVICE_EXTRA_ROW);
        ...
        if(cursor.moveToNext()){
            if(cursor.getInt(0)==1){
                ...
                i.putExtra(TimerService.EXTRA_SERVICE_REPEATING,true);
                startService(i);
